...
Second platform (2026-09-07): `daylight.login` is registered as a CODE ACTION, not an agent action. Daylight's login is a Django CSRF form POST that works over plain HTTP, so it runs no browser and no LLM and costs zero Browserbase minutes. This follows the existing rule that a browser is used only where HTTP genuinely fails, which for LightReach is Auth0. The two installers (wolfpack, 2ndcity) are separate 1Password items and separate sessions: Daylight scopes every job to the logged-in installer and 404s cross-installer reads, so there is deliberately no "default" client and the handler refuses one. The caller (job-automation) stores the returned cookie in its own encrypted per-tenant row and does all its reading itself; the gateway holds only the credential.
...
| `src/portals/lightreach/`             | LightReach code action: Browserbase login (Auth0 defeats pure HTTP), then browserless HTTP reads over the 24h cookie jar, per-client PolitenessBudget                                                                                                |
| `src/portals/daylight/`               | Daylight code action `daylight.login`: mints an installer portal session. No browser and no LLM, because Daylight is a plain Django CSRF form login that works over fetch; the gateway's value here is the credential boundary, not automation       |
| `src/portals/politeness.ts`           | process-scoped per-client login and read budgets                                                                                                                                                                                                     |
| `src/gateway/secrets.ts`              | JIT 1Password resolution per platform.client; 60s username/password cache, OTP never cached, single-flight, minimal op env                                                                                                                           | , accepts a bare item name (resolved in `OP_PORTALS_VAULT`) or a full `op://vault/item` reference for logins that live elsewhere |
